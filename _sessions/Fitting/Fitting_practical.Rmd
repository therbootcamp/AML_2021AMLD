...
So far we have only used one feature (`accommodates`), to predict `price`. Let's try again, but now we'll use a total of three features: 
...
# updated recipe
airbnb_recipe <- 
  recipe(XXX ~ XXX +  XXX + XXX, data = XXX)
```

```{r}
# updated recipe
airbnb_recipe <- 
  recipe(price ~ accommodates + bedrooms + bathrooms,
                        data = airbnb)
```
...
4. Refit the model as you have done above, and call it `price_lm`.
...
5. Using the `tidy()` function on the `price_lm` object, take a look at the parameter estimates.
...
6. Using the `predict()` function, to extract the model predictions and bind them together with the true values using `bind_cols()`.
...
7. Using the following code, plot the fitted against the true value, to judge how well our model performed. What do you think, is this performance good or bad? And how does it compare to the model with only one feature we fitted before?
...
8. Using the `metrics()` function, evaluate the model performance. Pass it the `price` variable as `truth` and the `.pred` variable as `estimate`.
...
9. How do you interpret these values? How do they compare to the ones you obtained previously?
