...
1. Open your `RBootcamp_AMLD2021` R project. It should already have the folders `1_Data` and `2_Code`.
...
```{r}
airbnb_split <- initial_split(airbnb, stata = price, prop = .5)
```
...
2. Use `control_resamples()` to set up the info for our resampling function such that it will save the predictions and the workflow. To do so, set the `save_pred` and `save_workflow` arguments to `TRUE`. Call the output `keep_pred`.
...
2. Create a regression model using the `lm` engine and the regression mode.
...
3. Create a workflow called `lm_workflow` using `workflow()` and add the `lm_recipe` and `lm_model` objects.
...
4. Fit the model on the training data. Here, using resampling doesn't make any sense, as we are not tuning any parameters. Use the `fit()` function that you already know from previous practicals and save the output as `lm_res`. We will compare the model performances afterwards.
...
```{r}
# Fit the regression model using resampling
lm_res <-
  lm_workflow %>% 
  fit(airbnb_train)
```
...
1. Define a recipe called `ridge_recipe`. Use the same recipe as with the regression above (`lm_recipe`), but add an additional pre-processing step: `step_normalize(all_numeric_predictors())`. This is to ensure that all parameters are penalized equally.
...
2. Create a ridge regression model, again using the `linear_reg()` model, but this time with the  `glmnet` engine. Within `linear_reg()`, set the argument `mixture` to 0 and `penalty` to `tune()`. Save the output as `ridge_model`
...
3. Create a workflow called `ridge_workflow` using `workflow()` and add the `ridge_recipe` and `ridge_model` objects.
...
4. Set up a grid of tuning parameters. Here we only have one parameter `penalty`. Use `grid_regular()` to do so, and pass it `penalty()` as first argument and `levels = 50`as second argument. Call it `penalty_grid`.
...
6. Use `collect_metrics()` to evaluate the cross-validation performance of the model across the different values of the penalty parameter.
...
7. OK, that's kind of hard to interpret, given that we'd have to scan 100 rows. Instead, let's plot the `penalty` value on the x axis, and the performance metrics on the y axis. Use the code below to do so.
...
8. Hm, the tuning did not seem to have a large effect. But the maximum penalty value automatically chosen by the `grid_regular` function was only 1. Specify your own grid of parameter values and repeat steps 5 to 7 above.
...
```{r}
penalty_grid <- tibble(penalty = 10 ^ (seq(0, 3, length = 150)))
...
9. Now we see a drop in the RMSE at higher `penalty` values. Select the best `penalty` value by passing `ridge_grid` into the `select_best()` function and save the output as `best_ridge`.
...
10. Now let's finalize our workflow using the `finalize_workflow()` function. Pass it the `ridge_workflow` and the `best_ridge` objects as first and second arguments. Save the output as `final_ridge`.
...
11. Now we can fit the model using the final workflow. Use the `fit()` function to this end, by passing it the `final_ridge` workflow and `airbnb_train`. Save the output as `ridge_res`.

```{r}
ridge_res <- fit(final_ridge, airbnb_train) 
```

12. Use the `tidy()` function to look at the parameter values of `ridge_res`.

```{r}
tidy(ridge_res) 
```

13. The `last_fit()` function, which you can use with the `final_ridge` workflow instead of the `fit()` function, let's you directly evaluate the model performance. Pass the `final_ridge` and `airbnb_split` (! instead of the `airbnb_train`) objects into `last_fit()` and directly pipe the output into `collect_metrics()`.
...
```{r}
penalty_grid <- tibble(penalty = 10 ^ (seq(-1, 1, length = 150)))
```
...
```{r, echo = TRUE}
complexity_grid <- tibble(cost_complexity =  seq(from = 0, to = .01, length = 50))
```

5. To speed up the model tuning, execute the following code (this will parallelize the tuning process).
...
|host_since| Experience of the hosts in days |
