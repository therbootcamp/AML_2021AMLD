...
```{r, eval = FALSE, echo = TRUE}
penalty_grid <- tibble(penalty = seq(0, 50, length.out = 200))
```
...
```{r, eval = FALSE, echo = TRUE}
penalty_grid <- tibble(penalty = seq(0, 10, length.out = 200))
```
...
6. Tune the model on the training data. To do so using our resampling scheme, use the `tune_grid()` function and pass it the defined workflow as first argument and set the `resamples` argument to `airbnb_folds`, and the `grid` argument to `complexity_grid`. Save the output as `dt_grid`. (Note: This will take some time...)
